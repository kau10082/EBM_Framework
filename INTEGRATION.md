...
| 生產接線 | `EBM_Search/SEARCH_SPEC.md`（Phase 1 ⑦、跨 Phase 停頓點 7、v0.20 changelog） |
...
| 框架指引／打包 | `CLAUDE.md`（工作根、路徑前綴、單一入口、config）；`pack_framework.py`（打包成單一 skill ZIP）｜可當 Claude Code 專案，亦可匯入 Claude Desktop（skill `ebm-framework`） |
