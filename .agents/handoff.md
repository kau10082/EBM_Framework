...
【本輪性質】**改善計畫（尚未實作，待使用者核准）** — 根治「手機/遠端執行 repo 時 8 項規範漂移」。
（前一塊 strategy_adherence_check 守門已實作並結案，commit `117d023`；本區覆寫為新計畫。）

**本輪審查範圍（檢視計畫合理性；尚無新程式碼）：** 本 handoff「待審查」區的計畫本身；涉及的現有檔＝
`EBM_Search/SEARCH_SPEC.md`、`.claude/skills/ebm-search|ebm-analysis/SKILL.md`、
`EBM_Search/scripts/{gate_guard,stage1_check,report_check,leg_exhaust_check,strategy_adherence_check}.py`、
`EBM_Analysis/tools/{verify_all,audit_consistency,prisma_audit,build_reports,validate}.py`、`.claude/settings.json`。
...
- 計畫：g0_strategy.json 擴充記錄每軸同義詞集；新增 **axis_coverage_check**：每腿 query 須涵蓋各必含軸 ≥N 個同義詞/別名/代號，過度簡化即 FAIL。SKILL 硬步驟「四軸展開表先寫成 g0_strategy.json 再開搜」。
...
- 計畫：report_check 增「prisma_flow 區塊必存在且含 identification/screening/included 數」；新增 **pdf_emitted_check**：宣稱 Phase1 完成前，PDF 須實際存在於 pdf_output_dir（檔在且非空）。SKILL 明寫「無 PDF 不算 Phase1 完成」＋PDF 必含節清單。
