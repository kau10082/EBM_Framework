...
4. **(🟡 欄寬溢出，使用者追報)** AMSTAR2 欄內容（綜整名＋關鍵領域敘述）比 RoB2 短判定詞長，原寫死窄欄(0.08)
   文字溢出 → 第 2 段欄寬亦改**資料驅動** `rob_section.colwidths`（無則回退原預設）；本案設 `[0.11,0.12,0.12,0.11,0.13,0.12,0.08,0.21]`，
   並把資料中不可斷行的斜線拉丁串(eos/IgE/…)改頓號。以 fitz 實際柵格化第 2 段表格目視確認無溢出。
