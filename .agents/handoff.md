...
- (5) `classify_units.py` 樞紐表修正（本輪，使用者抓出）——`PIVOTAL_LABALAMA_ARM["TRIVERSYTI"]` 由 `True` 改 `False`：TRIVERSYTI ＝ BDP/FF/G **vs BDP/FF（＝ICS/LABA）**，對照非雙支擴 LABA/LAMA，應與 FULFIL/TRILOGY 同列**非核心**；先前 curation 誤標 True 使其混進核心。另在 ⑤b review-flags 新增**矛盾偵測**：凡 `core_basis=pivotal_trial_design` 但摘要對照疑為 ICS/LABA（`versus inhaled corticosteroid…`）且無雙支擴訊號者，自動列待覆核，逼人工核對權威表（避免下次再有表項 curation 錯誤靜默放行）。
