...
✅ 已修：PDF『格式跑掉』＝我誤用 A4 直式，SEARCH_SPEC 與使用者範本都是 **A4 橫向(landscape) 容寬表**。修 `build_search_pdf.py`：`pagesize=landscape(A4)`、W 改用橫向寬度 → 5 欄核心表/6 欄背景表/PRISMA 流程圖皆不再擠壓；用 pymupdf 實際 render 比對範本確認版面正常（流程圖左為階段方塊縱列＋箭頭、右為引文追蹤臂/排除側欄）。註：範本為 Study 級摘要(主報告＋『另含N篇』)＝3 頁；本版依現行 report_check『子報告逐篇、禁另含N篇』故較長(23 頁)＝內容granularity 差異，非格式 bug；若要回到範本的精簡 Study 級摘要需放寬 report_check 規則，留使用者裁決。

✅ 已做：把規範版 PDF 產生器固定成 committed 通用工具 `scripts/build_search_pdf.py`。緣由：原本 ⑥ PDF 我手寫成簡化摘要、未照 SEARCH_SPEC 報告規範（缺 PRISMA 流程圖、核心表只列計數沒逐篇、無 APA/交叉檢核/全文狀態欄、清單三/未納入來源不全）——使用者指正。本輪修改：(1) 先用 report_check 契約欄位確定性組出 `_search_report.json`（studies 逐 Study 逐篇報告[標題,PMID,DOI,全文狀態,交叉檢核]、background 6 欄、ongoing_trials、funnel_closure 二分算式、prisma_flow；APA metadata 由 efetch 取真實作者/年/刊名，不杜撰）→ **過 report_check ✅**；(2) 新增 `build_search_pdf.py`：讀 `_search_report.json` 全資料驅動產固定章節①–⑪＋PRISMA 流程圖(reportlab.graphics)＋逐 Study 報告表＋APA，**字型 fallback**（Windows msjh.ttc→Linux wqy-zenhei→Noto→Helvetica），本機與雲端皆可產；(3) gate_guard：報告版型/內容、Phase1 PDF 實體 皆 ✅。實測雲端 reportlab＋WenQuanYi 正常出 19 頁中文 PDF。SEARCH_SPEC 補「標準 PDF 產生器」一條（資料 build_report_data→report_check 與 版面 build_search_pdf 分離、皆可機器稽核）。註：build_report_data.py 既有但預期 cache 檔名(g7_final_decision/g1_ctgov)與本 pipeline 不同，本案直接照 report_check 契約組 _search_report.json；可後續統一檔名。
