...
把 Gemini 稽核抓到的內部矛盾編成確定性檢查（非靠模型自律）：
  ...
  …C4-C16（全文依賴/NNTH/相對絕對並列/運算覆驗/MID/HR絕對欄/證據體/critical 必列/
      跨呈現一致/NNT CI/受試者數一致/concern 須註明）。
  C17 降級必附腳註：任一結局確定性<高，synthesis.sof_footnotes 須非空（Ch14 §14.2）。
  C18 ≥2 試驗須有 meta_analysis 段（Ch10 數據綜整；未池化亦須 model=not_pooled+理由）。
退出碼非 0 表示有矛盾，供 CI / 渲染前擋關。
（C17/C18＝2026-06 使用者定『analysis 標準成品格式』之硬 gate：每份報告務必含降級腳註＋統合分析段。）
"""
import json, sys, re
...
def check(syn=None):
    ...
    # C17: 降級必附腳註——任一結局確定性 < high，synthesis.sof_footnotes 須非空
    #      （GRADE 降級透明，Ch14 §14.2；2026-06 使用者定為標準格式：SoF 須附降級腳註 a/b/c…）。
    certs = [o.get("certainty") for o in (syn.get("sof") or [])] + \
            [b.get("certainty") for b in (syn.get("body_of_evidence") or [])]
    if any(c and c != "high" for c in certs) and not (syn.get("sof_footnotes") or []):
        fails.append("C17 有結局確定性<高卻無 sof_footnotes（GRADE 降級須附腳註說明 a/b/c…，Ch14 §14.2；標準格式鐵律）")
    # C18: ≥2 試驗須有統合分析段（Ch10 數據綜整）——meta_analysis 必含（池化或 not_pooled+理由）。
    #      2026-06 使用者定標準格式：第三段須逐核心結局給池化合併效應＋I²；未池化亦須列 model=not_pooled+理由。
    if len(syn.get("rob_summary") or []) >= 2 and not (syn.get("meta_analysis") or []):
        fails.append("C18 ≥2 試驗卻無 meta_analysis（Ch10：須逐核心結局給池化合併效應+I²，未池化亦須 model=not_pooled+理由；標準格式鐵律）")
    return fails
