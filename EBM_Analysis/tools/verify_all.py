...
依序執行並彙總：
  ...
  2) selfcheck_consistency  自我一致性硬 gate C1-C18（C15＝SoF 受試者數一致性防 N 飄移；C17 降級必附腳註；C18 ≥2 試驗須含統合分析段）
...
def main():
    ...
    try:
        ...
        fails = sc.check()
        ok = not fails
        results.append(("selfcheck C1-C18", ok, "\n".join(fails)))
        print(f"  {'✅' if ok else '❌'} selfcheck C1-C18" + ("" if ok else f"（{len(fails)} 失敗）"))
