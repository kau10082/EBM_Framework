...
>   3. **Tier 3（實取全文：Crossref 摘要欄 ＋ PMC 全文）**：重判。切題→切題；**判得出且有內容→離題**；Tier 3 仍取不到可判內容→**強制升 Tier 4（不得在此逕判『皆無』）**。**★ PMC 全文路徑鐵律（committed，不靠記性）：PMC 全文一律走 NCBI `efetch db=pmc`（解析 JATS `<body>` 正文）＝可重用器 `scripts/pmc_fulltext.py` 的 `NcbiClient.fetch_pmc_body`，嚴禁用 EuropePMC `fullTextXML`（對本流程不穩/常空）。** 該器**內建 NCBI E-utilities 速率限制（無 api_key＝3 req/s、有 key＝10 req/s）**——超速會被 429 整批靜默失敗（實測：Tier 3 一度 sleep 0.2s＝5 req/s→全部 efetch 回 None→0 篇全文）。非 OA-subset 的 PMC 文章 efetch 只回 front matter（無 `<body>`）→ 視為『PMC 無全文』、續走 Tier 4。**嚴禁再為此手刻 efetch／自訂 sleep；一律 import 此器。**
...
> > **★ 本關＝廣蒐去重，不含引文搜索(鐵律,2026-06 使用者糾正):** 第①關只做「各腿用核准 query 廣蒐 → 跨腿去重」,**不做引文追蹤/滾雪球(citation search/snowball)**。引文追蹤＝**第④關**,且須在 ③ 嚴格篩定出核心(清單二)後、以核心為種子才做(見 §④、§1① OpenAlex 引文追蹤腿)。OpenAlex／Europe PMC 在 ① **僅作廣檢**,其 references/citations 引文鏈能力留待 ④;① 的逐腿報告**勿讓使用者誤以為本關已做引文搜索**。
...
> **② → 停**,報告**接下來的篩選策略**(必含連言軸、可用的四型軸、納入/排除準則;含「初步篩選＋嚴格篩選」兩階),等使用者確認後才篩。**★ SR Filter 不在本②關問(時機鐵律,2026-06 使用者糾正)：** SR Filter 的詢問與決定**屬第⓪關策略時機(廣蒐前)**,因為它會改變『廣蒐要送哪些腿/子腿』。本②關**不得首次提出或補問** SR filter；若⓪關漏問,須**退回⓪關補記** `sr_filter_decision` 並(必要時)補跑對應 `<leg>-SR` 子腿廣蒐,**不可在已開始篩選的②關才補**。(本鐵律因實測把 SR filter 詢問誤置於②關而立;與 `check_sr_filter_decided`〔廣蒐產物存在時 `sr_filter_decision` 須已決定〕對稱。)
> **②b 初步篩選(高敏感,寧留勿殺)完成 → 停**,報告**初篩狀況**(保留 N、明顯離題剔除 N 及各自缺哪軸);只移除「某核心軸明顯離題」者、模糊先留,等確認後才進。**★ ②b 必須以『標題＋摘要』高敏初篩,嚴禁只憑標題(鐵律,2026-06 使用者糾正;Cochrane/MECIR 高敏感紅線,與 ④ 的 `citation_screen_check` 對稱)：** 許多 RCT/SR 標題用成分名／縮寫／廣義詞(chronic lung disease／airflow obstruction／inhaled therapy；PK/沉積研究只寫藥名),只看標題會漏殺隱藏的相關研究。故 ②b **須先批次抓回摘要**(efetch 200/批、EuropePMC 批次),以『標題＋摘要』判定;唯確實無摘要可抓者(登錄試驗、會議摘要、抓取後確認無摘要)標明 `abstract_status` 後可只用標題＋登錄欄位判。已落為機器 gate `gate_guard.py` `check_2b_abstract_screen`(腳本 `scripts/screen_2b_abstract_check.py`):`g2b_screen.json` 須結構化宣告 `screening_method="title+abstract"`、`abstracts_fetched>0`、`title_only_dropped==0`,且**任一被剔除且有 ID 的記錄須有摘要證據**(`has_abstract=true` 或 `abstract_status∈{none_after_fetch,registry,conference}`);否則(含舊版純 list/只憑標題)＝FAIL。**★ 停頓點另由機器 gate `check_2b_stop`(併入 `gate_guard`)看守(2026-06 使用者糾正『②b 完成未停下報告即逕跑 ③』而立,與 `check_strategy_approved`「⓪→① 防搶跑」對稱)：②b 產物 `g2b_survivors.json` 完成後須停下報告、經使用者點頭,於 `g2b_checkpoint.json` 設 `approved_by_user:true` 才可進 ③;`g3_FINAL_screen.json` 已產出但 ②b 未核准＝③ 搶跑 → FAIL。**
...
> > **★ ③ 內部『逐 Tier 停頓報告』(鐵律,2026-06 使用者定版)：** ③ 的分層升級 **Tier 1(摘要)→ Tier 2(CT.gov 登錄欄/AI 合成摘要)→ Tier 3(實取全文)→ Tier 4(Unpaywall→定案)**,**每一層之間都必須停下來報告該層結果(本層新判幾筆切題、幾筆升級、軸命中分布)、經使用者核准後才可跑下一層**;**嚴禁一個「繼續」就把 T2→T3→T4 連跑到底**(尤以 Tier 3/4 全文抓取量大、可能誤判,必須讓使用者逐層看過)。落地:每層完成寫 `g3_tierN.json`,使用者核准後在 `g3_tierN_checkpoint.json` 設 `approved_by_user:true`;Tier 4 收斂寫 `g3_FINAL_screen.json`。已落為機器 gate `gate_guard.py` `check_screen_tier_stops`:下一層產物存在但上一層 checkpoint 未核准＝跨 Tier 搶跑 → FAIL(與 `check_2b_stop` 等停頓 gate 同構)。
...
> **★ ④/⑤a/⑤b 三關各須『停下報告、經使用者核准後才續』(2026-06 使用者定版,機器看守,與 ⓪→①、②b→③ 對稱)：** 不可一個「繼續」就把 ④引文追蹤→⑤a交叉驗證→⑤b決定納入單位→⑥ 一口氣跑完。三個停頓點由機器 gate 看守(併入 `gate_guard`)，使用者核准後分別在 cache 寫 checkpoint：
> > - **④→⑤a**：`check_citation_stop`——⑤a 產物 `g6_verified.json` 已產出但 `g4_checkpoint.json` 未 `approved_by_user:true` → FAIL（④ 引文追蹤後須先停、報告、核准）。
> > - **⑤a→⑤b**：`check_xref_stop`——⑤b 產物 `g7_units.json` 已產出但 `g6_checkpoint.json` 未核准 → FAIL（⑤a 交叉驗證/撤稿後須先停、報告、核准）。
> > - **⑤b→⑥**：`check_units_stop`——⑥ 產物 `_search_report.json` 已產出但 `g7_checkpoint.json` 未核准 → FAIL（⑤b 決定納入單位後須先停、報告、核准）。
...
>   > **★ ⑦ 分類須讀『標題＋摘要(方法學)』、非只靠標題/pubtype(v0.21):** 決定設計型態(RCT/SR-MA/觀察/指引/經濟/綜述)與對照臂(是否 LABA/LAMA)**必須核對摘要方法學內容**;只靠標題會把常用字試驗名誤併(實測 `IMPACT` 當常用字→59 報告灌水,改 word-boundary＋排除『IMPACT of/on』＋NCT 號歸併後降為合理值),靠 pubtype 又因 DOI-only 缺 pubtype 而大量『未分型』。已落為可重用腳本 `scripts/classify_units.py`(讀 g6_verified＋g2c 摘要＋聯集 NCT,輸出 g7_units.json):設計優先序 SR/MA→指引(僅 pubtype/標題,不掃 abstract 之 recommendation 以免誤併)→計畫書→RCT→觀察→經濟→綜述;RCT 以 **NCT 號優先**、其次 word-boundary 試驗縮寫歸併為 Study,並判對照臂含 LABA/LAMA 與否區分『核心:三合一 vs LABA/LAMA』。**殘餘(未辨識試驗的 RCT 報告、未分型背景)據實申報為待人工覆核**(rapid-review 定位,未達 MECIR 雙人獨立)。
>   > **★ ⑦ 標準固定步驟(v0.21,通用、由 g0 軸驅動,適用任何主題):** ⑦ 已收斂成可重現的固定流程,以 `scripts/classify_units.py --cache <dir> --enrich` 一道指令完成:
>   > **(1) 內容先到位**:進 ⑦ 的每筆都應有摘要/全文(②c 已 materialize；abstract-first：摘要→PMC/EPMC 全文→Unpaywall)。**嚴禁對 title-only 硬分類**——有內容才判型態(與 ③ 同一份內容,不得自相矛盾)。
>   > **(2) 設計判別**:讀『標題＋摘要(方法學)』,優先序 SR/MA→指引→計畫書→RCT(含 post hoc/次級分析→歸該試驗)→觀察→經濟→藥學/裝置→共識/調查→綜述→其他次級。**終端一律給確定背景類,不留『無法分類』桶**。
>   > **(3) RCT→Study 連結**:**NCT 為最可靠鍵**(任何 NCT 皆自成一 Study,`--enrich` 線上以 CT.gov 補名)→其次 word-boundary 試驗縮寫(排除常用字 of/on)→再次『樣本數特徵＋摘要內縮寫』保守連結。連不到＝獨立小研究/會議摘要→據實標待人工,**嚴禁臆測硬連到樞紐試驗(誤歸比留白更糟)**。
>   > **(4) 介入相關性檢核(非核心歸背景，不剔除)**:`--enrich` 對每個 NCT 查 CT.gov `InterventionName`,**以 g0 的 I 軸同義詞／四軸展開成分判該試驗介入是否在範圍內**;不在範圍者(他藥/雙合一,如生物製劑/單方/dual)移『**背景:非核心RCT**』(歸表三背景、不計入核心)，**不丟棄**。**此判準由 g0 軸驅動→換主題自動適用,不改碼。**
>   > **★ ⑤b 後不設『剔除』桶(鐵律,2026-06 使用者糾正)：** 凡通過 ③ 切題(P∧I∧C 已成立)者,⑤b **只分『核心(表二納入)／背景(表三)』兩類,不得再有『剔除/排除』桶**——已篩到 ⑤b 還在排除＝等於否定 ③ 的切題判定、且會無聲丟掉已查證文獻。reading-methodology 發現非核心三合一vs雙合一者(他藥 RCT、無 LABA/LAMA 對照臂、純綜述/指引/觀察/經濟…)一律**歸背景(表三)**,corpus_seed＝表二+表三**全保留**。對帳恆等式相應為 **Σ核心研究報告 ＋ SR/MA ＋ 背景 ＝ 切題總數**(無剔除項)。
>   > **★ 核心 RCT 判定邏輯(定版,鐵律,2026-06 使用者逐筆糾正後立;`classify_units.py` 實作,⑤b 須以 `--enrich` 執行)：** 「核心原始 RCT」＝**真正完整發表、且對照臂含 C 軸(本題＝LABA/LAMA 雙支擴)的隨機分派試驗**。判定分兩層、缺一不可:
>   > **(L1) 設計判別『是不是 RCT』(先擋偽 RCT)**:`is_rct_pt`(RCT pubtype)最可靠;**無 pubtype 時,先以 `R_REVIEW_STRONG`(narrative review/「to review」/「we searched」/drug profile…)與 `R_PK_STRONG`(population PK/bioequivalence/gas trapping…)擋下『綜述體/藥動』**——因綜述常引述他人試驗而含 `randomized/placebo-controlled` 字樣、會誤觸 `R_RCT`;`R_OBS` 另含 before-after/pharmacoepidemiology 擋觀察性;**『其他原始研究』回退路徑須真有隨機化證據 `R_RAND` 才算 RCT**。**★ 研究計畫書(無結果)不得當核心**:`R_PROTO_STRONG`(study protocol/rationale and design/results expected/will be randomised/first patient 20XX…)強訊號**蓋過 R_RCT**(protocol 必含 randomized 字樣)→ 歸『進行中/試驗計畫書(待結果)』,**只有結果報告才進核心**(否則下游 GRADE 誤以為有可評讀數據)。
>   > **(L2) 核心/非核心(對照軸 C)**:① **會議摘要(`R_CONF_DOI`/`R_CONF_TITLE`:congress-/conference/poster/synopsis)且非已納入樞紐子報告 → `待評估:會議摘要`,不得當核心**(MECIR『studies awaiting classification』)。② **已知樞紐試驗以 trial-level 權威表 `PIVOTAL_LABALAMA_ARM` 定案**(核心/非核心是試驗『設計』屬性、不隨單篇子報告摘要飄移:IMPACT/ETHOS/KRONOS/TRIBUTE 有 LABA/LAMA 臂＝核心;FULFIL/TRILOGY(vs ICS/LABA)、TRINITY(vs 單方)＝非核心)。③ **非樞紐才回退 regex `trip∧dual`**:`dual`(C 軸)偵測前**先正規化分隔符(en/em-dash→hyphen、β→b)＋遮蔽三合一藥名跨度(`R_TRIP.sub`)**,避免 `glycopyrrolate–formoterol` 漏判(假陰)與 `FF/UMEC/VI⊃UMEC/VI` 子字串誤命中(假陽)。④ **NCT 經 `--enrich` 查 CT.gov 介入確認非三合一 → `背景:非核心RCT`**。**★ ICS 退階/移除設計另立子型**:`R_ICS_WD`(withdrawal of ICS/de-escalation/step-down/discontinue ICS…)命中之核心試驗改記 `核心:ICS 退階試驗`＋`design_subtype=ICS-withdrawal`(SUNSET/WISDOM)——回答『能否撤 ICS』≠『起始三合一 vs 雙支擴』,**下游 meta 不得與起始試驗混算方向性**。
...
>   **(a)(b) 都確認後才進 ⑥。**
> **⑥ PDF 報告交付（5 段固定版型）→ 停**。**★ 版型＝使用者 2026-06-24 定版的『5 段核心』（committed，取代舊三表/PRISMA-流程圖版型；嚴禁手拼 _search_report.json 或自創版型）：1 檢索基本參數(PICO/檢索日期精確到日/資料庫清單/限制條件) → 2 具體檢索策略·完整字串(各腿逐字布林，MECIR 可重製) → 3 PRISMA 流程數字(每階段 起始→排除原因·數→剩餘) → 4 最終納入證據清單(只核心，固定欄位＝`作者／年份／文獻類型 ｜ 標題 ｜ DOI ｜ PMID ｜ PubMed/Crossref 驗證`；背景不列表) → 5 進行中試驗(`登錄號 NCT ｜ 標題`)。** 流程＝`build_search_report_data.py --cache <dir> --out _search_report.json`（由 cache 確定性組資料、schema＝`references/search_report_schema.json`）→ `build_search_report.py --in _search_report.json`（渲染、字形淨化、不硬編題目）。**無 PDF 不算 Phase 1 完成。**
> > **★ PDF 一律可產，不得以環境為由略過(2026-06 使用者糾正)：** `build_search_report.py` 已內建跨平台字型 fallback(msjh.ttc→**wqy-zenhei.ttc**→Noto CJK→Helvetica)，**Linux/遠端/容器同樣產得出中文 PDF**(實測 `/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc` 即可)。缺 reportlab 就 `pip install reportlab`；無 Windows 輸出資料夾就輸出到 cache 或當前目錄。**嚴禁宣稱『此環境無法產 PDF』而略過**——PDF 是 Phase 1 完成的硬要件。
> > **★ PDF 表格長字串必換行(2026-06 使用者糾正)：** PRISMA 漏斗表『納入分析』明細(基底 SR/MA＋佐證 RCT 清單)等長字串，**必以 `cell()`(Paragraph, wordWrap=CJK) 包每格**餵 reportlab Table，不可餵純字串——純字串不換行會溢出、與下一段(最終納入清單)重疊。`build_search_report.py` 已落實(funnel 表 `fr` 以 `cell()` 包)。
> > **★ PRISMA『納入分析』明細列法(2026-06 使用者定版)：** included_for_analysis 各列 **(a) 標籤不夾方法學附註**(不要寫『(grade=full；Phase 0 以 AMSTAR2/CCA 定…)』這類括註，標籤只留乾淨類別名如「Overview 基底 SR/MA」「佐證原始 RCT」)；**(b) 研究以『第一作者 + 年份』標示，不用 PMID**(如「Hu 2026；Calderón-Montero 2025」)。方法學說明(grade_track／Phase 0 流程)留交接包與內文，不塞進 PRISMA 明細欄。
...
> > **★ 機器守門優先於記性(鐵律 v0.20.3,因『取盡偷工』『漏跑 Unpaywall』各復發而立):** 凡有對應守門腳本的關卡,**報告該關完成前必先跑守門、貼 PASS 才算通關**——不可只靠記憶/自律。守門對照:**Gate ① 取盡**＝`scripts/leg_exhaust_check.py`(讀 `g1_legs_manifest.json`,逐腿斷言可窮盡腿 fetched≥hitCount;廣蒐時就要逐腿寫 hitCount/fetched/exhaustible/skipped+reason 進 manifest)、**Gate ②c Unpaywall**＝`scripts/fulltext_audit.py`/`gate_guard.py` 的 Unpaywall 覆蓋稽核(每筆非全文且有 DOI 必須已過 Unpaywall 且未誤判)、**流程數字**＝`funnel_check.py`；**Gate ③ 反坍縮**＝`gate_guard.py` `check_screen_partition`（前身名 check_partition_provenance）(每筆配穩定唯一 `uid`、全程只用 uid 當鍵；嚴禁用 `pmid+'|'+doi` 之類複合鍵當 dict key——無 ID 記錄會全坍縮成同一鍵造成污染。守門以 uid 獨立重算:screened⊎awaiting 恰覆蓋 base 且互斥無重複、且每筆 screened 須真有 abstract 或在 fetched 表)。**判 have 須實抓驗證**＝`verify_have_fetchable.py`＋`gate_guard.py` `check_have_verified`：與 `fulltext_audit.py`(查 not-have 是否其實 OA、防低估)對稱——**凡 included 且要評讀(full/targeted_harms)、fulltext_status=have(online 無本機 pdf)者,必須以 `verify_have_fetchable.py` 實抓確認『真全文』(PMC/PDF/夠長且含方法-結果-CI 多特徵的 HTML;單純 OA 旗標或中繼著陸頁不算)、蓋 `fulltext_verified=true`**;否則守門 FAIL。**嚴禁只憑 Unpaywall `is_oa` 旗標判 have**(實測 IMPACT/ETHOS 旗標 OA 但 NEJM 出版商 URL 403、ETHOS 還因錯 DOI 假陽性,拖到評讀才爆;此器在 ②c/⑧ 就抓出假 have、降 need-supplement)。**`have(online)` 之全文取得性要窮盡 Unpaywall `oa_locations`(非只 best),別只試一個來源。**
> > **★ 公開 API 檢索腿一律 import `scripts/public_legs.py`，嚴禁每輪手刻 urllib(鐵律,2026-06 使用者糾正後立;與 `pmc_fulltext.py` 同精神)：** Gate ① 的 OpenAlex／Europe PMC／ClinicalTrials.gov／PubMed(E-utilities) 取得**必須**走可重用器 `public_legs.py`(`fetch_openalex`／`fetch_europepmc`／`fetch_ctgov`／`fetch_pubmed`)，**不得每輪重寫 ad-hoc 取得碼**——手刻 urllib 正是兩個復發 bug 的根因：**(BUG-1 編碼)** 參數值內預先放 `%20` 會被 `urlencode` 二次編碼成 `%2520` → API 收到壞字串 → **0 筆**(實測 OpenAlex `benralizumab%20asthma` 回 0)；器內一律以 `urlencode` 單次編碼、多詞 AND 用半形空格表示。**(BUG-2 取盡)** OpenAlex `meta.count` 是**估計值**、常與翻頁實際數差 1~2，**取盡判定看 `next_cursor` 是否為 None**(非 `fetched>=count`)，取盡後 `hitCount` 回報 `fetched`、估計值另存 `count_estimate`(否則 `leg_exhaust_check` 會因差 1 筆誤判 FAIL)。安裝/clone 後 `python scripts/public_legs.py --selftest` 線上自證(編碼正確且 >0 筆)。
> > **★ ③ 分層篩選一律 import `scripts/screen_tiers.py`，嚴禁每輪手刻軸判定(鐵律,2026-06 Antigravity 複審『強烈建議』後立;與 public_legs.py 同精神 shift-left)：** ③ 的『單筆內容→軸判定→verdict』與『寫 g3_FINAL 前自驗』**必須**走可重用器 `screen_tiers.py`(`judge_axes`／`verdict`／`finalize_check`)，**不得每輪重寫**——手刻 tier 腳本正是三個復發 bug 的根因：**(G1 無內容判定)** `judge_axes` 無 abstract/全文者一律回 unknown→escalate，**嚴禁憑標題判 present→切題**(實測:Tier3 對 6 筆 title-only 憑標題同時含三軸即判切題)；**(G2 鍵名)** axis_hits 鍵直接取 g0 軸名(`P_disease/I_benralizumab/…`)、非通用 P/I/C(實測:strict_screen_check 因鍵名不符報放水)；**(G3 provenance)** `finalize_check` 在寫 `g3_FINAL_screen.json` 前自驗、切題/離題須帶 abstract/fulltext_excerpt/registry/ai_summary，否則攔下(實測:最終檔只存 axis_hits→來源證明 gate 報 741 筆無內容卻有判定)。內容抓取(PMC/Unpaywall/AI 合成)仍由呼叫端以 `pmc_fulltext.py`／`fulltext_exhaust.py`／Consensus·OE 完成。`judge_axes` 已內建**分隔符正規化＋跨軸長詞優先遮蔽**(SEARCH_SPEC §98)防『對照軸短同義詞⊂介入軸長同義詞』假命中；但**對照軸(C)同義詞在 g0 必須是『該藥專屬』、嚴禁放類別泛詞**(如 `anti-IL-5`／`anti-interleukin-5` 同屬 benralizumab=anti-IL-5R 的描述→會在不同 span 誤命中 C→假切題；2026-06 複審實測 CALIM[benra vs placebo] 被誤判切題 29 筆)——對照軸要用 INN/品牌/代號(mepolizumab/Nucala/SB-240563)。`python scripts/screen_tiers.py --selftest` 自證三保證＋遮蔽＋分隔符＋fetched-proof。
> > **報告資料制式化**＝`build_search_report_data.py`(確定性從 cache 組三表、**固定欄位**〔核心 5 欄/背景 6 欄/進行中 3 欄〕、**每格務實填滿**〔缺 title/全文狀態/登錄號自動 EuropePMC/Unpaywall 回填〕、排除撤稿、寫出前自我驗證無缺格；`--merge-into _search_report.json`)。**嚴禁再手拼 _search_report.json 的三表**(手拼＝欄位飄、常缺格的根因)——一律走此產生器。
> > **報告版型/內容**＝`gate_guard.py` `check_search_report_format`＋`check_flow_closure`(③只能切題/離題二分、核心表 PMID 欄不得空、研究名不得佔位『待確認/請見』、子報告逐筆不得『另含N篇』、背景表須含 PMID＋檢核欄、進行中試驗表必存在、閉合須含二分算式——皆為 2026-06 使用者逐條糾正之缺失轉成的機器檢查)。**撤稿管控**＝`gate_guard.py` `check_no_retracted`：⑤a交叉驗證**統一查撤稿**（PubMed `Retracted Publication`[PT]＋標題 `Retracted:`＋**Crossref `is-retracted`/`update-to:retraction`**），RETRACTED 一律剔出 納入/背景/報告表/**Zotero payload**/交接包（實測撤稿 SR 36072633 曾被當背景匯入 Zotero→須先剔除再匯入；已匯入才發現則以 Zotero API 刪除）。
> > **★ ⑤a 必查『DOI↔標題一致性』，嚴禁只查存在性(鐵律,2026-06 使用者糾正後立)：** Consensus／OE 腿不回 DOI、Claude 手填易錯——錯 DOI 因『指向的論文真的存在』而會被純存在性驗證判 VERIFIED 漏網(實測：13 篇 base NMA 有 3 篇 DOI 指向 Campylobacter／Omalizumab／Phase-2 trial 等不相干論文，造成重複與抓錯全文，到 Phase 0 才被使用者抓到)。故 ⑤a 對每筆有 DOI 的納入候選**必須再查 Crossref 該 DOI 的實際標題、與記錄標題比對**(committed `xref_verify.py` 本就做；輕量獨立稽核器＝`scripts/doi_title_audit.py`，`audit(records)`／`--in records.json`／`--selftest`)，similarity 低於門檻＝DOI 可能填錯→標 UNVERIFIED/待修，不得當 VERIFIED 放行。**嚴禁用『只查 DOI 是否存在』的簡化自製 ⑤a 取代標題比對。** **★機器 gate（Antigravity 第七輪 🔴 而升，不再只靠本鐵律）：** ⑤a 須以 `python scripts/doi_title_audit.py --in <納入候選> --out <cache>/g6_title_audit.json` 把稽核結果落檔；`gate_guard.check_doi_title_audited` 在 `g6_verified.json` 存在且含有 DOI 的納入候選時，要求 `g6_title_audit.json` 存在且 `mismatches` 為空，否則 FAIL（gate 離線讀檔，連網比對在 ⑤a 落檔時做）。
> > **★ 可攜可靠性(鐵律 v0.20.4,因『記憶不隨 repo、別人 clone 會重犯』而立):** auto-memory 是**個人本機、不進 repo、不被 pack_skill 打包**→別人拿不到、不可作為唯一防線。**真正可攜的防線＝committed 進 repo 的 SEARCH_SPEC＋scripts/{gate_guard,leg_exhaust_check,fulltext_audit,funnel_check}＋專案級 `.claude/settings.json`(Stop hook,用 `${CLAUDE_PROJECT_DIR}` 相對路徑)。** 凡新建守門腳本**必 commit**(未追蹤＝別人沒有)。**每關報告完成前自己先 `gate_guard.py --cache <dir>` 並貼 PASS**(打包成 Desktop skill 時無專案 hook,只能靠此自律＋SKILL 指示)。**安裝/clone 後先 `python scripts/selftest_guards.py`** 證明守門有效。**每當使用者人工抓到新缺失,優先把它轉成 gate_guard 一條檢查(而非只寫記憶)。**
> > **★ 防『未查全文就丟兩者皆無』(機器 gate,v0.21;因 2026-06 使用者糾正而立)：** ②c 對初篩保留者**逐筆抓全文**，**無全文且無摘要才列待評估**。但「待評估」分兩種、不可混：**有 DOI/PMID＝有全文路徑**，必須先查 Unpaywall／PMC，若仍取不到內容→判 `待人工補全文`(`channels_exhausted=true`，記 OA 連結)，**嚴禁逕判 `兩者皆無`**；`兩者皆無` 僅限**完全無 ID／無任何全文路徑**者。（v0.21 曾落為 `stage1_check`，該 gate 已於 v0.22 移除；現行由 `gate_guard.check_nocontent_bucket` 看守：判『全文及摘要皆無』者須帶實抓證明旗標，且有 DOI 者須 `unpaywall_checked=true`。）
> > **★ 軸比對品質守則(v0.21;因 2026-06 實跑 ETHOS/KRONOS 誤判離題、FULFIL/TRILOGY 誤判切題而立)：** ③ 逐軸比對同義詞時 **(a) 必先正規化分隔符**——slash／hyphen／en-dash(–)／em-dash／逗號／『 and 』一律視為等價、β→b，並用**詞幹**比對(`dual therap` 同時涵蓋 therapy/therapies)，否則 `glycopyrrolate–formoterol` vs `glycopyrrolate/formoterol`、`dual therapy` vs `dual therapies` 會漏判→樞紐試驗被誤剔(實測 ETHOS/KRONOS)。**(b) C(對照)軸嚴禁用『會成為三合一名稱子字串的雙支擴藥對』當訊號**(如 `umeclidinium vilanterol`⊂FF/UMEC/VI、`glycopyrrolate formoterol`⊂BGF)，亦不可用『two long-acting bronchodilators』(那是在描述三合一本身的組成)——否則 三合一 vs ICS/LABA 試驗(FULFIL/TRILOGY)會被誤判『有雙支擴對照』。C 軸改靠**通用詞幹(dual therap／dual bronchodilat／LABA-LAMA)＋雙支擴品牌(Anoro/Ultibro/Stiolto/Spiolto/Duaklir/Bevespi)＋成分不在常見三合一內的乾淨藥對(indacaterol/glycopyrronium、tiotropium/olodaterol、aclidinium/formoterol)**。**(c) 關責提醒**：③ 只判『主旨切題(P∧I∧C 字樣是否現)』，**對照臂究竟是不是 LABA/LAMA 屬 ⑦ 決定納入單位**時讀方法學才定案——故 ③ 寧可放進(切題)再於 ⑦ 歸類，勿在 ③ 用關鍵字硬判對照臂而誤剔。
> > **★ 防『③ 只憑摘要 punt 成待評估』(機器 gate,v0.21;因 2026-06 使用者糾正而立)：** 進 ③ 的候選**都已有內容**(摘要，多數有全文)，③ 必須用**全文/摘要**做出 `切題/離題` 二元判定。若摘要看不出對照 C，**須抓全文核對**後再判；**僅當『實際抓過全文仍無法核對』才可掛 ③待評估**(`fulltext_checked`/`oa_fetch_attempted`/`channels_exhausted`)。（v0.21 曾落為 `check_screen_awaiting_resolved`，該 gate 已於 v0.22 隨 g2c 廢除；現行由 `check_excl_requires_fulltext`（離題須 Tier3 實取全文）與 `check_nocontent_bucket` 看守。）(本規補強 spec 原『abstract 看不出 C→待評估』：不得只停在摘要，須先抓全文。)
> > **★ 防『有 OA 連結卻不抓就丟待評估』(機器 gate,v0.21;因 2026-06 使用者糾正而立)：** 原則＝**能 trace 到全文/摘要者一律進下一關**，只有「線上真的取不到任何可篩內容」才掛待評估。OA 全文是**可抓取管道**，不可只記 `oa_url` 就掛待評估——**必須實際下載並嘗試解析 OA 全文**(PDF→pypdf／HTML→meta+正文)；取得內容→升回候選，解析失敗才掛 `待人工補全文` 並標 `oa_fetch_attempted=true`。（v0.21 曾落為 `stage1_check`，已移除；現行：`fulltext_exhaust.py` 實抓後蓋 `oa_fetch_attempted`/`oa_urls_tried`，由 `check_nocontent_bucket` 稽核。）
> > **★ ②c 必『實抓+解析』全文，不可只憑 OA/PMC 旗標標 have(機器 gate,2026-06 使用者糾正兩 Bug 而立)：** ②c 判「有無可篩內容」時，對**無摘要**者**不可只看 `inEPMC`/`isOpenAccess`/`pmcid`/`is_oa` 旗標就標 `have/有全文`**——旗標≠抓得到、≠解析得出正文。必須**實際窮盡管道抓取並解析**：(1) PMC 全文＝NCBI `efetch db=pmc`(經 `pmc_fulltext.py`，解析 JATS `<body>`；**不用 EuropePMC fullTextXML**)；(2) **Unpaywall 全部 `oa_locations`(非只 best)**逐一抓；(3) OA PDF(pdftotext/pypdf)／HTML(去標籤)解析。**取得可解析正文(夠長、能比對軸詞)才算 have**；**三管道全部抓+解析後仍無可篩內容者，一律在『②c 這一關』判 `待評估(待人工補全文)`，不得漏進 ③**。外文(非英文)正文若無法以英文軸詞可靠比對，亦歸 `待人工補全文`(需人工/翻譯)。**(Bug1：無摘要無可解析全文者，應在②c就歸待評估、不該留到③才發現；Bug2：未盡力解析全文＝偷工，全文會『突然』在後關才解析得出。)** 已落為機器 gate `gate_guard.py` `check_screen_partition`（前身名 check_partition_provenance）強化：凡 screened(進 ③)且無 abstract 者，其 uid 必須在 `g3_fetched_by_uid.json` 且帶**實抓解析證明**(`text_len`≥1500 或 `verified`/`fulltext_verified`，登錄試驗 `channel:registry` 例外)；只掛旗標無正文＝FAIL。此即把「have 須實抓驗證」從後段納入核心(verify_have_fetchable)**前移到 ②c 內容分類**。
...
> > **★ 報告策略時須主動詢問是否套用『Systematic Review Filter』(鐵律,2026-06 使用者糾正)：** 報告檢索策略、等核准的同時,**必須主動問使用者「是否要套用 Systematic Review Filter(系統性回顧/方法學過濾器)」**,並說明**分工**:
> > **此『有沒有問過 SR filter』已落為機器 gate `gate_guard.py` `check_sr_filter_decided`(與 `check_strategy_approved` 對稱)**：Stage A 廣蒐(`g1_legs_manifest.json`)產出時,`g0_strategy.json` 的 `sr_filter_decision` 必須是已決定值(`applied`／`declined`／`not_applied`／`none`),**停在 `pending` 或缺漏即 FAIL**——把「報告策略時漏問 SR filter」(2026-06 使用者糾正)從靠記性變機器看守。決定前 `g0` 先寫 `sr_filter_decision:"pending"`,得到使用者答覆後才改為已決定值再開始廣蒐。
> > - **PubMed 腿維持 Cochrane 高敏感 RCT 過濾器**(撈原始 RCT;此腿不套 SR 過濾器)。
> > - **SR 過濾器套在『PubMed 以外』的搜尋腿(Consensus／OpenAlex／Europe PMC)**,用來補抓既有的**系統性回顧/統合分析(SR/MA/NMA)**:Consensus 用 `study_types=["systematic review","meta-analysis"]`(AI 合成腿以結構化參數限定);**OpenAlex／Europe PMC 的 SR 過濾器必須是『複合語法』——『控制詞彙(出版類型 PubType／主題詞 MeSH/Emtree)＋自由文字詞(Title/Abstract)』兩成分並用,不可只勾出版類型**。
> > - **★ SR filter 不可只靠出版類型(鐵律,2026-06 使用者補正;機器看守 `sr_filter_composite_check`)：** 正確的 SR 檢索過濾器是**經設計測試的複合策略**,把 SR 專屬詞(`systematic review`／`meta-analysis`／`network meta-analysis`)**同時**套在自由文字欄(Title/Abstract)與控制詞彙(出版類型/MeSH),例如標準 PubMed 寫法 `((systematic review[pt] OR meta-analysis[pt] OR systematic review[tiab] OR meta-analysis[tiab]))`。**嚴禁只用出版類型(Publication Type)過濾**,因為:(1) **索引時間差(indexing lag)**——資料庫人工貼 PubType/MeSH 需數天～數月,只用 PubType 會漏掉剛發表、尚未被索引的最新 SR/MA;(2) **索引不一致**——作者用語與索引者認知可能不同。**Cochrane MECIR C33 強制:任何方法學過濾策略都須並用『控制詞彙(PubType/MeSH)』與『自由文字詞(Title/Abstract)』**。此規由機器 gate `gate_guard.py` `check_sr_filter_composite`(腳本 `scripts/sr_filter_composite_check.py`)看守:每條 SR 子腿(`<leg>-SR`／role=SR_MA_NMA)且用 Boolean query 者,其實際 query 須同時含控制詞彙成分與自由文字成分,缺任一＝FAIL;AI 合成腿(Consensus/OE,以 study_types 限定)豁免。
> > **★★ 三成分強化(2026-06 使用者再糾正;`sr_filter_composite_check` 已落地、`selftest_guards` 有回歸)：** 使用者定版要求＝**「出版類型(PubType) ＋ 主題詞(MeSH/Emtree) ＋ 自由文字(Title/Abstract)」三成分複合**,且機器看守補強兩點(先前漏接、致 EuropePMC-SR 一度只有 PubType＋裸詞就過關)：(1) **自由文字必須『欄位綁定』Title/Abstract**(`[tiab]`／`TITLE:`／`ABSTRACT:`／`title_and_abstract.search:`)——**裸詞不算**,因 EuropePMC REST／OpenAlex 預設搜『全文』,裸詞會把全文泛提及撈進來＝噪音、非 Title/Abstract;(2) **凡所屬 DB 索引 MeSH/Emtree(PubMed/MEDLINE、EuropePMC、Embase…)者,SR filter 必須真的並用 MeSH 成分**(`"…"[mesh]`／`MESH_TERM:"…"`),不可只用『出版類型＋自由文字』略過主題詞;DB 真無 MeSH 索引者(如 OpenAlex,用 concepts/topics 非 MeSH)須於 g0 該 leg 標 `mesh_unavailable:true` 並註明理由＝誠實豁免,不偽造 MeSH 欄位。(故上方 PubMed `[pt]＋[tiab]` 範例僅示意控制＋自由文字二成分;**用於 MeSH-capable DB 的 SR 腿時必須再補 `[mesh]` 主題詞成分才達三成分**。)
> > - **ClinicalTrials.gov 為試驗登錄庫,SR 過濾器不適用**(只有試驗、無 SR/MA)。
> >
> > **★ SR 過濾器分工：DB 腿無過濾主檢不得進篩選語料庫(2026-06 使用者定版,取代先前『additive 聯集進既有池、不取代廣檢』寫法)：** 仍對非 PubMed DB 腿『額外』跑 SR 子查詢,但**進入篩選語料庫(screening corpus／`g1_union.json`)時,凡有 SR 變體的非 PubMed DB 腿(Consensus／OpenAlex／EuropePMC)只採其 `<leg>-SR` 結果,嚴禁把該腿『無過濾主檢』結果灌進語料庫**——因為這些 DB 腿的無過濾主檢(尤以 EuropePMC REST 預設**全文檢索**)會帶進大量全文泛提及噪音(實測 EuropePMC 主檢 3816 筆多為內文/參考文獻泛提及)。**分工：原始 RCT 由 PubMed 腿(Cochrane RCT 過濾器)負責;SR/MA/NMA 由 DB 腿的 SR 變體負責;登錄試驗由 ClinicalTrials.gov 負責。** DB 腿的無過濾主檢若有實跑,只作為『廣蒐取盡/稽核紀錄』寫進 manifest(leg_exhaust 仍據此驗取盡),**不進語料庫**。**此規由機器 gate `sr_division_check`(併入 `gate_guard`)看守**：SR filter 啟用(g0 `sr_filter_decision:"applied"` 或 legs 含 `<leg>-SR`)時,`g1_union.json` 任一筆 provenance(legs) 含『有 SR 變體之非 PubMed DB 腿的主檢(同 base、非 -SR)』即 FAIL。(本鐵律因 triple vs dual COPD 案把 EuropePMC 無過濾主檢全文噪音灌進語料庫而立。) 此為策略階段使用者決策,不可預設替他決定;問法白話、附建議。決定後寫進 `g0_strategy.json`(`sr_filter_decision:"applied"`):**被套 SR 過濾器的『非 PubMed』腿用 `<leg>-SR` 子腿表示、設 `design_filter_allowed:true`**(strategy_adherence gate 才放行該腿 query 內的方法學過濾詞;PubMed 腿照舊),並在報告「檢索原則」節載明。
> > **★ 四軸展開必須真的做(機器 gate,v0.21;因 2026-06 使用者糾正而立)：** 四軸展開(§1①(1))是鐵律——每個概念都要沿『縮寫↔全文／臨床慣稱↔生化別名／藥類↔INN↔開發代號↔品牌／疾病縮寫↔全文』把別名窮盡求最大 recall。既有 `axis_coverage_check` 只驗『每腿 query ≥1 同義詞命中(軸存在性)』,**攔不到『同義詞庫根本沒展開』**(例：P 只寫 COPD、I 只寫 triple therapy 也會通過 coverage)。故另立 `gate_guard.py` `check_axis_expansion`(腳本 `scripts/axis_expansion_check.py`)直接稽核 **g0_strategy.json 的 axes 同義詞庫本身**：對每條 in_query/mandatory_screen 軸,須(1)去重後同義詞 ≥3、(2)至少 1 個全文/多詞形式(證明做了縮寫↔全文展開);任一不滿足＝四軸展開未做/過度簡化＝FAIL。g0 存在即稽核(⓪ 策略階段就生效)。採低門檻(不要求塞滿 N 個),與 axis_coverage 設計一致避免 fail-closed。(本鐵律因 triple vs dual COPD 案第一版策略同義詞過於稀疏、未做四軸展開而立。)
> > **★ 對照軸純度：檢索 query 只含 in_query 軸(P＋I)，C 軸不得進 query(機器 gate,v0.21;因 2026-06 使用者連兩版糾正而立)：** 廣蒐階段求**最大 recall**——每腿 query **只能含 `in_query=true` 的軸**(一般＝疾病軸 P＋介入軸 I)。**對照軸 C(`in_query=false`)等任何「不放進 query」的軸,其同義詞嚴禁出現在任何腿的 query**;把對照/排除詞塞進 query 會砍掉『標題/摘要沒提對照組』的研究、嚴重傷 recall(對照軸要留待 ③ 嚴格篩讀全文才比對)。此要求已落為機器 gate `gate_guard.py` `check_comparator_purity`(腳本 `scripts/comparator_purity_check.py`)：比對前先遮蔽所有 in_query 軸同義詞(避免 I 軸長詞如 `ICS/LABA/LAMA` 內含 C 軸子字串 `LABA/LAMA` 造成誤判),殘餘 query 出現任何 in_query=false 軸同義詞＝FAIL。manifest 優先、無 manifest 時退回 g0.legs，故 ⓪ 策略階段就會被稽核。(本鐵律因 triple vs dual COPD 案 Consensus／OpenAlex query 連兩版被擅自塞入「versus dual therapy LABA/LAMA」而立。)
...
> > **★ Stop hook 必須找得到 cache 才有用(機器自我驗證,v0.21;因 2026-06 實測 hook 靜默失效而立)：** Stop hook 跑 `gate_guard.py --auto --hook` **不帶 `--cache`**,靠 `_find_cache(None)` 自動發現。**主幹發現法＝掃 repo 內 `EBM_Search/cache/*/` 找帶 `_search_active.flag` 的進行中 cache**(`_find_active_cache_by_flag`),哨兵旗標本身就是『此 cache 有檢索進行中』的地真值,**與 run_state／`EBM_CACHE_DIR`／OneDrive 路徑無關**。先前只靠 run_state＋硬編 Windows 路徑 `~/OneDrive/文件/...` → 在 fresh-clone／手機／非 Windows／cache 不在預設位置時 `_find_cache(None)` 回 `None` → `_active(None)=False` → hook **靜默 exit 0**,自動守門等同未啟用(實測本案 cache 在 `EBM_Search/cache/<topic>/` 即整輪 hook 全程休眠,gates 僅靠人工 `--cache` 才跑到)。`selftest_guards.py` 已加回歸:旗標 cache 找得到、無旗標回 None(休眠)。
> > **〔❗歷史紀錄（v0.21），已於 v0.22 廢除——本段所述 Stage A/B 架構、`build_stage1_corpus.py`、`stage1_check.py`、`_stage1_corpus.json` 均已移除，勿執行；現行單一流程見檔頭 v0.22 定版〕** 原文：把易漂移的檢索中段釘成磁碟邊界——
...
> > **Stage B＝③嚴格篩→④引文追蹤→⑤a交叉驗證→⑤b決定納入單位**（編號依上方定版；交叉驗證/決定納入單位＝⑤ 子步，非 ⑥/⑦）,**只讀 `_stage1_corpus.json`**(看不到 Stage A 原始混沌),輸出＝`_corpus_seed.json`(verdict 定案 included/background)。
> > **此切割直接根治本框架最常犯的「待評估屬②c還③」邊界混淆**;但切段只是必要非充分,段內失誤仍靠各 gate_guard 檢查——兩者互補。
> > **★ 待評估只在 ②c 產生，③ 必須二元(鐵律,v0.21;因 2026-06 使用者再三強調而立)：** **「待評估」(awaiting classification) 只有 ②c 這一關會產生**。②c＝判「有無可篩內容」,依**最省成本的三段管道序**逐一查、命中即停:**(1) 有無摘要**(有摘要即有內容→進 ③);**(2) 無摘要者→全文是否可線上閱讀**(PMC／Europe PMC `inEPMC/isOpenAccess/pmcid/hasPDF`→有即進 ③);**(3) 仍無者→全文是否可經 Unpaywall 等 OA 機制線上取得**(`is_oa`→有即進 ③);**三管道全部失敗者才踢到待評估**(Stage A,擱置、不進 ③;有 ID/全文路徑但三管道取不到內容＝`待人工補全文`、完全無 ID＝`兩者皆無`)。此序避免對大量初篩保留者全部抓全文。**（v0.21 曾落為 `check_awaiting_channels`/`awaiting_channels_check.py`，已於 v0.22 移除；現行對應＝`check_nocontent_bucket`：實抓證明旗標 `abstract_checked ∧ fulltext_parse_attempted ∧ channels_exhausted`＋有 DOI 須 `unpaywall_checked`。）**進入 ③ 的都已有內容,故 **③(g3_FINAL_screen) 必須是『切題/離題』二元,不得再出現待評估**。（v0.21 曾落為 `check_awaiting_stage`，已移除；現行 g3 verdict 枚舉由 `check_screen_partition` 直接稽核＝{切題,離題,全文及摘要皆無}。）**本鐵律澄清並取代先前散見的『③待評估』寬鬆措辭**:③ 模糊個案一律歸離題(清單三-B 標模糊),不得在 ③ 新生待評估。
> > **★ 關責不外溢(總則,鐵律 v0.20.1):** 每一關**只做該關定義的事,不把後段關的工作提前**。這是本管線反覆出現的失效模式——已實測兩次:**第⓪關(策略)**曾擅自列「排除準則」(排除是第②/③關才做)、**第③關(嚴格篩)**曾擅自做「文獻型態分類/決定納入單位」(那是 ⑤b)。**自我檢查徵兆:** 若我在某關的輸出開始出現「下一關才該有的詞」(策略關出現『排除』、嚴格篩出現『納入單位/RCT/SR-MA/背景型態』、引文追蹤前出現『verdict』),即為外溢,**立即收回、退回本關該有的輸出**。各關產物邊界一覽:⓪只定義『要搜什麼·什麼算切題』;①只廣蒐去重(不篩主旨/不驗證);②只報篩選策略(不動手篩);②b高敏感初篩(只砍明顯離題);②c只記全文狀態(不剔除);③只判主旨切題與否(不分型態);④才做引文追蹤;⑤a才交叉驗證、⑤b才決定納入單位(verdict/study);⑥才出三表/PDF;⑦才寫交接包。(編號依上方「管線步驟編號（定版）」；交叉驗證/決定納入單位＝⑤ 子步，不是 ⑥/⑦。)
...
- **② 參考文獻追蹤(C30,反向引文)**:對**納入研究 ＋ 相關綜述／SR(含被排除的敘述綜述)**掃參考清單(OpenAlex `referenced_works`／Europe PMC references / OA cited_by 正向),補抓藏在其中的原始 RCT;於報告註記已掃對象與新增筆數。被排除的敘述綜述仍有此「剩餘價值」。
  > **★ 鐵律(v0.21,Cochrane 高敏感紅線):引文追蹤新候選必須以『標題＋摘要』高敏初篩,嚴禁只憑標題丟。** 滾雪球滾出的新候選**不得**用「標題沒出現 P/I 字串就直接丟、不抓摘要」來篩——許多 RCT 標題用成分名(`fluticasone furoate/umeclidinium/vilanterol`)／縮寫／廣義詞(`chronic lung disease`／`airflow obstruction`／`inhaled therapy`),只看標題會漏殺隱藏 RCT,正好抵銷引文追蹤的目的(Cochrane 初篩＝檢視『標題與摘要』、從寬納入)。**技術正解＝Batch API 批次抓摘要**(PubMed efetch 200/批、OpenAlex 50/批、EuropePMC),數千篇摘要只需 ~10–20 次呼叫,再對全部新候選做『標題＋摘要』篩——**不是**為了省 API 就犧牲敏感度。唯一例外:**完全無摘要可抓**者用「負向排除法」(只在標題明顯他題 cancer/pregnancy/animal 時丟、其餘從寬放行),不可正向硬性要求標題命中。已落為機器 gate `gate_guard.py` `check_citation_screen`(腳本 `scripts/citation_screen_check.py`):讀 `g4_citation_tracking.json`,逐輪斷言 `screened_on` 含 title+abstract、`title_only_dropped==0`、有 ID 新候選>0 時 `abstracts_fetched>0`。(本鐵律因 2026-06 實作一度為避免逐筆抓摘要太慢而改『只看標題丟』、被使用者糾正而立;實測:同題以批次抓摘要改回標題＋摘要篩,新增切題由 0→19。)
...
> **★ 標準 PDF 產生器(v0.21,committed,通用):** 不必每案重寫——用 `scripts/build_search_report.py --in _search_report.json --out <報告.pdf> [--font <路徑>]`：讀『經 gate_guard check_search_report_format 驗過的』`_search_report.json`，**全資料驅動**(固定章節①–⑪＋PRISMA 流程圖＋逐 Study 逐篇報告表＋APA＋背景/進行中/清單三/限制)，換主題不改碼。**字型自動 fallback**：Windows `msjh.ttc`／`msjhbd.ttc` → Linux `wqy-zenhei.ttc` → Noto CJK → (缺則 Helvetica＋stderr 警告)，故**本機(Windows)與雲端(Linux)皆可產**(實測雲端用 WenQuanYi 正常出中文)。前置：`_search_report.json` 由 `build_search_report_data.py`(或等效)確定性組出、過 gate_guard 該 gate。流程＝資料(build_search_report_data→過 gate_guard 的 check_search_report_format)＋版面(build_search_report)分離,兩段都可機器稽核。
> 產生器(若一案自寫亦可,zero-dep reportlab)。**輸出資料夾讀根 `config/settings.yaml` 的 `report.pdf_output_dir`**(現＝`OneDrive\文件\EBM_Framework\reports`)——絕對路徑含本機使用者名＝個資,故**只放根 settings.yaml(gitignored)、不進 repo/ZIP**;留空才回退解析 Windows『文件』已知資料夾 `[Environment]::GetFolderPath('MyDocuments')`(正確處理 OneDrive 重導與中文資料夾名)。檔名建議 `<主題>_report.pdf`。**⚠️ 勿用 `%USERPROFILE%\Documents`**——OneDrive 接管(KFM)下真正「文件」常是 `…\OneDrive\文件`,`%USERPROFILE%\Documents` 會指到非同步空資料夾、使用者在檔案總管看不到(實測踩坑);產出後**回報絕對路徑**給使用者。
