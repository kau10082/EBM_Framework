...
> **★ 管線步驟編號（定版，唯一權威；以下停頓點為準，勿再用別套編號）：① 廣蒐去重 → ② 篩選策略 → ②b 初篩 → ②c 全文取得性 → ③ 嚴格主旨篩選 → ④ 引文追蹤 → ⑤ 收斂後處理〔⑤a 交叉驗證(Crossref/PubMed 存在性＋撤稿剔除)、⑤b 決定納入單位(classify_units：Study 歸併／核心 vs 背景／剔他藥)、⑤c Zotero 匯入、⑤d 人工補全文資料夾〕 → ⑥ 三表＋PDF 報告 → ⑦ 交接包 `_corpus_seed`＋問是否續進 EBM 分析。** （`④→⑤→⑥→⑦`，中間無跳號；交叉驗證與決定納入單位＝⑤ 的子步，**不是 ⑥/⑦**——⑥/⑦ 固定為「三表+PDF」「交接包」。先前 spec 散見「⑥交叉驗證／⑦決定納入單位」係舊草編號筆誤，以本定版為準。）
...
>   > **(4) 介入相關性檢核(剔他藥)**:`--enrich` 對每個 NCT 查 CT.gov `InterventionName`,**以 g0 的 I 軸同義詞／四軸展開成分判該試驗介入是否在範圍內**;不在範圍者(他藥/雙合一,如生物製劑/單方/dual)移『排除:非介入範圍 RCT』,不計入核心。**此判準由 g0 軸驅動→換主題自動適用,不改碼。**
