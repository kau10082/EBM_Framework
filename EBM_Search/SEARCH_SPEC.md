...
> > **★ 防『③ 只憑摘要 punt 成待評估』(機器 gate,v0.21;因 2026-06 使用者糾正而立)：** 進 ③ 的候選**都已有內容**(摘要，多數有全文)，③ 必須用**全文/摘要**做出 `切題/離題` 二元判定。若摘要看不出對照 C，**須抓全文核對**後再判；**僅當『實際抓過全文仍無法核對』才可掛 ③待評估**(`fulltext_checked`/`oa_fetch_attempted`/`channels_exhausted`)。已落為 `gate_guard.check_screen_awaiting_resolved`：`g2c_awaiting_classification.json` 內有 doi/pmid 卻無全文核對證明 → FAIL。(本規補強 spec 原『abstract 看不出 C→待評估』：不得只停在摘要，須先抓全文。)
> > **★ 防『有 OA 連結卻不抓就丟待評估』(機器 gate,v0.21;因 2026-06 使用者糾正而立)：** 原則＝**能 trace 到全文/摘要者一律進下一關**，只有「線上真的取不到任何可篩內容」才掛待評估。OA 全文是**可抓取管道**，不可只記 `oa_url` 就掛待評估——**必須實際下載並嘗試解析 OA 全文**(PDF→pypdf／HTML→meta+正文)；取得內容→升回候選，解析失敗才掛 `待人工補全文` 並標 `oa_fetch_attempted=true`。已落為 `stage1_check`：awaiting 帶 `oa_url` 卻無 `oa_fetch_attempted=true` → FAIL。
> > **★ ②c 必『實抓+解析』全文，不可只憑 OA/PMC 旗標標 have(機器 gate,2026-06 使用者糾正兩 Bug 而立)：** ②c 判「有無可篩內容」時，對**無摘要**者**不可只看 `inEPMC`/`isOpenAccess`/`pmcid`/`is_oa` 旗標就標 `have/有全文`**——旗標≠抓得到、≠解析得出正文。必須**實際窮盡管道抓取並解析**：(1) PMC 全文＝NCBI `efetch db=pmc`(經 `pmc_fulltext.py`，解析 JATS `<body>`；**不用 EuropePMC fullTextXML**)；(2) **Unpaywall 全部 `oa_locations`(非只 best)**逐一抓；(3) OA PDF(pdftotext/pypdf)／HTML(去標籤)解析。**取得可解析正文(夠長、能比對軸詞)才算 have**；**三管道全部抓+解析後仍無可篩內容者，一律在『②c 這一關』判 `待評估(待人工補全文)`，不得漏進 ③**。外文(非英文)正文若無法以英文軸詞可靠比對，亦歸 `待人工補全文`(需人工/翻譯)。**(Bug1：無摘要無可解析全文者，應在②c就歸待評估、不該留到③才發現；Bug2：未盡力解析全文＝偷工，全文會『突然』在後關才解析得出。)** 已落為機器 gate `gate_guard.py` `check_partition_provenance` 強化：凡 screened(進 ③)且無 abstract 者，其 uid 必須在 `g3_fetched_by_uid.json` 且帶**實抓解析證明**(`text_len`≥1500 或 `verified`/`fulltext_verified`，登錄試驗 `channel:registry` 例外)；只掛旗標無正文＝FAIL。此即把「have 須實抓驗證」從後段納入核心(verify_have_fetchable)**前移到 ②c 內容分類**。
...
> > **★ 待評估只在 ②c 產生，③ 必須二元(鐵律,v0.21;因 2026-06 使用者再三強調而立)：** **「待評估」(awaiting classification) 只有 ②c 這一關會產生**。②c＝判「有無可篩內容」,依**最省成本的三段管道序**逐一查、命中即停:**(1) 有無摘要**(有摘要即有內容→進 ③);**(2) 無摘要者→全文是否可線上閱讀**(PMC／Europe PMC `inEPMC/isOpenAccess/pmcid/hasPDF`→有即進 ③);**(3) 仍無者→全文是否可經 Unpaywall 等 OA 機制線上取得**(`is_oa`→有即進 ③);**三管道全部失敗者才踢到待評估**(Stage A,擱置、不進 ③;有 ID/全文路徑但三管道取不到內容＝`待人工補全文`、完全無 ID＝`兩者皆無`)。此序避免對大量初篩保留者全部抓全文。**已落為機器 gate `gate_guard.py` `check_awaiting_channels`(腳本 `scripts/awaiting_channels_check.py`):每筆待評估須標 `abstract_checked ∧ online_fulltext_checked ∧ unpaywall_checked ∧ channels_exhausted`,缺一＝FAIL(代表沒把三管道查盡就 punt);『兩者皆無』須真的無任何 ID/OA 路徑。**進入 ③ 的都已有內容,故 **③(g3_FINAL_screen) 必須是『切題/離題』二元,不得再出現待評估**。已落為機器 gate `gate_guard.py` `check_awaiting_stage`(腳本 `scripts/awaiting_stage_check.py`):g3 任一筆 verdict 帶待評估/待人工補全文/兩者皆無/awaiting＝FAIL。**本鐵律澄清並取代先前散見的『③待評估』寬鬆措辭**:③ 模糊個案一律歸離題(清單三-B 標模糊),不得在 ③ 新生待評估。
...
> **★ 標準 PDF 產生器(v0.21,committed,通用):** 不必每案重寫——用 `scripts/build_search_pdf.py --in _search_report.json --out <報告.pdf> [--font <路徑>]`：讀『經 report_check 驗過的』`_search_report.json`，**全資料驅動**(固定章節①–⑪＋PRISMA 流程圖＋逐 Study 逐篇報告表＋APA＋背景/進行中/清單三/限制)，換主題不改碼。**字型自動 fallback**：Windows `msjh.ttc`／`msjhbd.ttc` → Linux `wqy-zenhei.ttc` → Noto CJK → (缺則 Helvetica＋stderr 警告)，故**本機(Windows)與雲端(Linux)皆可產**(實測雲端用 WenQuanYi 正常出中文)。前置：`_search_report.json` 由 `build_report_data.py`(或等效)確定性組出、過 `report_check`。流程＝資料(build_report_data→過 report_check)＋版面(build_search_pdf)分離,兩段都可機器稽核。
