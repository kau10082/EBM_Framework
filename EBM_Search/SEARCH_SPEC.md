...
> > - **SR 過濾器套在『PubMed 以外』的搜尋腿(Consensus／OpenAlex／Europe PMC)**,用來補抓既有的**系統性回顧/統合分析(SR/MA/NMA)**:Consensus 用 `study_types=["systematic review","meta-analysis"]`;OpenAlex／Europe PMC 用 `PUB_TYPE` 或標題/摘要含 `systematic review`／`meta-analysis`／`network meta-analysis` 過濾。
