...
> > **★ 待評估只在 ②c 產生，③ 必須二元(鐵律,v0.21;因 2026-06 使用者再三強調而立)：** **「待評估」(awaiting classification) 只有 ②c 這一關會產生**。②c＝判「有無可篩內容」:**先看有無摘要**(有摘要即有內容→進 ③);**沒摘要者再看有無全文**(Unpaywall/PMC);**「無全文且無摘要」者才踢到待評估**(Stage A,擱置、不進 ③;有 ID/全文路徑但取不到內容＝`待人工補全文`、完全無 ID＝`兩者皆無`)。此「先摘要後全文」之序避免對大量初篩保留者全部抓全文。進入 ③ 的都已有內容,故 **③(g3_FINAL_screen) 必須是『切題/離題』二元,不得再出現待評估**。已落為機器 gate `gate_guard.py` `check_awaiting_stage`(腳本 `scripts/awaiting_stage_check.py`):g3 任一筆 verdict 帶待評估/待人工補全文/兩者皆無/awaiting＝FAIL。**本鐵律澄清並取代先前散見的『③待評估』寬鬆措辭**:③ 模糊個案一律歸離題(清單三-B 標模糊),不得在 ③ 新生待評估。
