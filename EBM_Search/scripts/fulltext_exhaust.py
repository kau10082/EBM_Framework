...
"""
import sys, os, re, io, json, time, argparse, urllib.parse, urllib.request, urllib.error
...
def _get_retry(url, timeout=60, tries=5, base=0.5):
    """_get ＋ 指數退避：對 429/503（NCBI/EuropePMC 限流／暫時故障）與暫時性 URLError 重試，
    退避 0.5→1→2→4→8s（5 次）；非限流類 HTTPError（如 404 非 OA）立即抛出、不重試。全失敗則抛最後一個錯。"""
    last = None
