...
def build_data(cache, mailto):
    ...
    g7recs = g7.get("records", []) if isinstance(g7, dict) else (g7 or [])
    ...
    for i in range(0, len(inc_pmids), 200):
        ...
        try:
            ...
            for pid in res.get("uids", []):
                ...
                a1 = au[0]["name"].split()[0] if au else ""
                doi = ""
    ...
    def verify_str(pid, doi):
        r = ver_by_pmid.get(str(pid)) if pid else (ver_by_doi.get(doi) if doi else None)
        if not r: return "—"
        v = r.get("verify", "—"); via = r.get("verify_via") or []
        if v == "VERIFIED":
    ...
    data = {
        ...
        "topic": g0.get("topic", ""),
        "search_date": g0.get("search_date", time.strftime("%Y-%m-%d") if False else ""),
        "params": {
