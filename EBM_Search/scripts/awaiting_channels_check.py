...
本守門讀 g2c_awaiting_classification.json，逐筆要求：
  ...
  • **且必須帶『完整實抓解析』的可稽核證明 `fulltext_parse_attempted=true`**（2026-06 使用者
    再糾正：移到待評估前必須真的下載並解析過全文，光標旗標/只看 is_oa 不算）。帶 doi 者
    另須有 `oa_urls_tried`（實際試過的 OA URL 清單，可為空＝Unpaywall 回無 OA）以證明有去抓。
  • 『兩者皆無』必須真的無任何 ID（doi/pmid/pmcid/nct/oa_url）且 abstract_checked。
缺旗標或自相矛盾＝FAIL（代表沒把三管道查盡／沒真的實抓解析就 punt 成待評估）。
此檢查對應 `scripts/fulltext_exhaust.py`：②c 須以該工具完整實抓解析、失敗者才寫入本檔。
...
def check(awaiting):
    ...
    for a in awaiting:
        ...
        if "兩者皆無" in reason:
            ...
            continue
        # 其餘待評估（有 ID＝待人工補全文）：三管道＋窮盡＋『完整實抓解析證明』缺一不可
        missing = [n for n, v in (("abstract_checked", abs_ck),
                                  ("online_fulltext_checked", onl_ck),
                                  ("unpaywall_checked/oa_fetch_attempted", oa_ck),
                                  ("channels_exhausted", a.get("channels_exhausted")),
                                  ("fulltext_parse_attempted", a.get("fulltext_parse_attempted"))) if not v]
        if missing:
            fails.append(f"[{pid}] 待評估但未證明三管道全查盡／未完整實抓解析，缺旗標 {missing}："
                         f"待評估只在『無摘要＋無線上全文(PMC/EPMC)＋Unpaywall OA 實際下載解析皆失敗』時才成立"
                         f"（須先以 fulltext_exhaust.py 完整實抓解析）")
        # 帶 doi 者須留實際試過的 OA URL 清單（可為空＝Unpaywall 回無 OA），證明真的去抓過
        elif a.get("doi") and ("oa_urls_tried" not in a):
            fails.append(f"[{pid}] 待評估（有 doi）但缺 oa_urls_tried：無法證明真的實際下載解析過 OA 全文"
                         f"（fulltext_exhaust.py 會寫入實際試過的 URL 清單；空清單代表 Unpaywall 回無 OA）")
    return fails
