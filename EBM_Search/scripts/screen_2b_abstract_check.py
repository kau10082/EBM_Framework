...
逐項斷言：
  ...
  • 比例守門（防『一律標 registry 規避抓摘要』）：被剔除且有 ID 者中，全靠 registry/conference
    『免抓摘要』豁免過關者，占比 ≥90% 且筆數 ≥8 → FAIL（保守閾值，避免誤殺 registry 多的主題）。
...
# 「免抓摘要」豁免狀態（registry/conference 宣稱本無摘要可抓）——濫用向量：一律標 registry 規避抓摘要。
SKIP_FETCH_STATES = {"registry", "conference"}
# 比例守門（保守，避免誤殺 registry 多的主題）：被剔除且有 ID 的記錄中，全靠 skip-fetch 豁免者
...
def check(g2b):
    ...
    if (removed_with_id >= SKIP_FETCH_ABUSE_MIN
        ...
        fails.append(
            f"②b 被剔除且有 ID 的 {removed_with_id} 筆中，{skip_fetch_drops} 筆"
            f"（{skip_fetch_drops/removed_with_id:.0%}）全靠 registry/conference『免抓摘要』豁免過關："
            "疑似以 abstract_status 規避批次抓摘要（等同退回只憑標題篩）。"
            "請確認確實批次抓過摘要、registry/conference 標記逐筆屬實")
    if title_only_drops:
