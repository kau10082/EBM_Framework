...
- **取全文不准便宜行事**：被判「讀不到」的文獻一律**強制再用 Unpaywall 查一次**，若其實有開放全文就擋下來、逼它去讀完整版（避免把讀得到的當二手）。
- **一鍵全驗**：定稿前一個指令把上面**一整套機器檢查**全部跑過、全綠才能出報告——甚至有一支檢查專門盯「程式現況有沒有跟自己留的開發筆記對不上」，免得靠過時的記憶辦事。
...
- **No cutting corners on full text**: any paper marked "can't be read" is **forced back through Unpaywall**, and if an open-access copy actually exists, it's blocked and made to read the full version (so nothing readable is treated as second-hand).
- **One-command full check**: before finalizing, a single command runs the whole battery of checks above — and the report is only produced when everything is green. There's even a check that watches whether the code's current state still matches its own development notes, so it never runs on stale memory.
...
評讀流程逐條對照國際標準 **Cochrane Handbook v6.5（全 21 章）** 與 **GRADE**；檢索流程對齊 **Cochrane 第 4 章與 PRISMA-S／PRISMA 2020**（敏感度優先、做引文追蹤、誠實聲明覆蓋限制）。報告含**證據摘要表（SoF）**、絕對效應與 **NNTB／NNTH（附信賴區間）**、**RoB 2 逐領域**、跨研究的**證據體 GRADE**、缺失證據敏感度與註冊庫對照。前述「自我查核」由 `selfcheck_consistency.py`（一致性 gate ＋ SoF 受試者數一致性）、`quote_verify.py`（逐字＋每個 SoF 數字回原文溯源）、`validate.py`（算術重算）、`render_smoketest.py`（版面/完整性＋渲染器一致性煙霧測試）、`prisma_audit.py`（PRISMA 2020 27 項完整度稽核）、`EBM_Search/scripts/funnel_check.py`（流程圖數字逐關閉合）、`EBM_Search/scripts/fulltext_audit.py`（強制 Unpaywall 複查）、`verify_all.py`（一鍵跑完整套）落實；另有 `verify_memory_claims.py` 把關「開發筆記 vs 程式現況」不漂移。**取全文的順序**是：先試 PMC 全文，再透過 **Unpaywall**（免金鑰、只要 email）找開放取用版本，真的讀不到才退而以「摘要＋臨床試驗登錄結果」合成、並**誠實標為二手**。框架本身也有 **pytest 單元測試＋GitHub Actions CI**（`tests/`、`.github/workflows/`），把工具的公式與檢查邏輯也納入自動測試。完整規格見 `EBM_Search/SEARCH_SPEC.md`、`EBM_Analysis/ANALYSIS_SPEC.md`，整合流程見 `INTEGRATION.md`。

The appraisal is cross-checked against the **Cochrane Handbook v6.5 (all 21 chapters)** and **GRADE**; the search aligns with **Cochrane Ch. 4 and PRISMA-S / PRISMA 2020**. Reports include a **Summary-of-Findings table**, absolute effects with **NNTB／NNTH (with confidence intervals)**, **per-domain RoB 2**, a cross-study **body-of-evidence GRADE**, a missing-evidence sensitivity analysis, and trial-registry cross-checks. The "self-checking" above is implemented by `selfcheck_consistency.py` (consistency gates + participant-count consistency), `quote_verify.py` (word-for-word source matching + per-number provenance), `validate.py` (arithmetic recompute), `render_smoketest.py` (layout/completeness + renderer-parity smoke test), `prisma_audit.py` (PRISMA 2020 27-item audit), `EBM_Search/scripts/funnel_check.py` (flow-diagram arithmetic), `EBM_Search/scripts/fulltext_audit.py` (forced Unpaywall re-check) and `verify_all.py` (one-command full battery); plus `verify_memory_claims.py`, which guards against the code drifting from its own development notes. **Full text is fetched in order**: PMC first, then **Unpaywall** (keyless, just an email) for an open-access copy, and only if it truly can't be read does it fall back to an "abstract + trial-registry results" synthesis, **honestly labelled as second-hand**. The framework itself has **pytest unit tests + GitHub Actions CI** (`tests/`, `.github/workflows/`), bringing the tools' formulas and check logic under automated testing too. Full specs are in `EBM_Search/SEARCH_SPEC.md` and `EBM_Analysis/ANALYSIS_SPEC.md`; the integration is in `INTEGRATION.md`.
