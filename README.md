...
評讀流程逐條對照國際標準 **Cochrane Handbook v6.5（全 21 章）** 與 **GRADE**；檢索流程對齊 **Cochrane 第 4 章與 PRISMA-S／PRISMA 2020**（敏感度優先、做引文追蹤、誠實聲明覆蓋限制）。報告含**證據摘要表（SoF）**、絕對效應與 **NNTB／NNTH（附信賴區間）**、**RoB 2 逐領域**、跨研究的**證據體 GRADE**、缺失證據敏感度與註冊庫對照。

前述「自我查核」在**評讀端**由 `validate.py`（逐階段 schema ＋ GRADE 算術重算）、`absrisk.py`（絕對效應公式黃金值自測）、`selfcheck_consistency.py`（一致性硬 gate C1–C18，含 SoF 受試者數一致性防 N 飄移）、`quote_verify.py`（逐字＋每個 SoF 數字回原文溯源）、`render_smoketest.py`（版面/完整性＋渲染器一致性煙霧測試 V1–V6）、`prisma_audit.py`（PRISMA 2020 27 項完整度稽核）落實，定稿前由 `verify_all.py` 一鍵全跑、全綠才放行。**檢索端**則由 `gate_guard.py`（一整排機器關卡：策略未核准防搶跑、各停頓點看守、撤稿管控、SR 分工等）統一守門，搭配 `funnel_check.py`（流程圖數字逐關閉合）、`fulltext_audit.py`／`fulltext_exhaust.py`（強制把全文取盡到 Unpaywall）、`sr_division_check.py`（系統性回顧分工），並以 `selftest_guards.py` 把這些關卡本身納入回歸測試。
...
On the **appraisal side**, the self-checking above is implemented by `validate.py` (per-stage schema + GRADE arithmetic recompute), `absrisk.py` (absolute-effect golden-value self-test), `selfcheck_consistency.py` (consistency hard gates C1–C18, including SoF participant-count consistency to prevent N-drift), `quote_verify.py` (word-for-word matching + per-number provenance), `render_smoketest.py` (layout/completeness + renderer-parity smoke tests V1–V6) and `prisma_audit.py` (PRISMA 2020 27-item audit) — all run before finalizing by `verify_all.py`, which only passes when everything is green. On the **search side**, `gate_guard.py` is the single gatekeeper (a whole row of machine gates: no jumping the gun before the strategy is approved, guarding each stop point, retraction control, SR division of labour, etc.), together with `funnel_check.py` (flow-diagram arithmetic), `fulltext_audit.py` / `fulltext_exhaust.py` (forcing full-text retrieval all the way through Unpaywall) and `sr_division_check.py` (systematic-review division of labour), with `selftest_guards.py` bringing those gates themselves under regression testing.
