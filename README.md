...
- **連版面和完整性也有機器把關**：自動檢查「該有資料的表格是不是空的」「章節有沒有跳號」「安全性必要結局（死亡、嚴重不良事件）有沒有漏列」「有沒有缺字形的亂碼方塊」——這些過去只能靠人眼校對的破圖，現在也由程式在出報告前擋下。
- **一鍵全驗**：定稿前一個指令把上面**十五道檢查**全部跑過，全綠才能出報告。
...
- **Even layout and completeness are machine-checked**: it automatically flags "a table that should have data but is empty", "a skipped section number", "a missing safety-critical outcome (death, serious adverse events)", and "tofu boxes from missing glyphs" — the kind of broken rendering that used to need a human eye, now blocked before the report ships.
- **One-command full check**: before finalizing, a single command runs all of the above — **fifteen checks** — and the report is only produced when everything is green.
