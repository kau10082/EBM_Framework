...
0. **搜尋策略報告**：先把四軸展開（縮寫↔全文／慣稱↔生化·基因別名／類別↔藥名(INN)↔代號／疾病縮寫↔全文）、各腿要送的**實際 query 字串**、**必含連言軸（＝納入/切題定義）**攤出來。**本關只定義「要搜什麼、什麼算切題」，不列『排除準則』、不做任何排除**——排除/篩選是第②關初篩與第④關離題篩才做的事（高敏感、寧留勿殺）。**先報告策略 → 停 → 等使用者確認 → 才開始廣蒐。**
   - **★ SR Filter 必問且時機＝本⓪關（廣蒐前），嚴禁拖到第②關（鐵律，2026-06 使用者定版）**：報告策略時**一併主動詢問**是否套用 **Systematic Review Filter**（系統性回顧／方法學過濾器；分工：PubMed 腿維持 RCT 過濾器抓原始試驗，SR/MA/NMA 由非 PubMed DB 腿的 `<leg>-SR` 子腿以複合語法補抓——詳見 `EBM_Search/SEARCH_SPEC.md`「主動詢問 SR Filter」段）。**SR filter 會改變『廣蒐要送哪些腿/子腿』，故必須在廣蒐前決定**，不能等開始篩選（②）才問。先把 `g0_strategy.json` 的 `sr_filter_decision` 寫 `"pending"`，得到使用者答覆後改 `applied`／`declined` 再開始廣蒐。機器守門 `gate_guard.py check_sr_filter_decided`：廣蒐產物 `g1_legs_manifest.json` 存在時 `sr_filter_decision` 仍 `pending`／缺漏即 FAIL。
...
7. **決定納入單位**（驗證標記後、補全文之前）：把納入候選定案分流——**原始研究**（以 **Study 為單位**，同一試驗多份報告連結成一筆，走 GRADE）／**背景參考**（MA/SR/指引/綜述/評論，不計入原始研究數、僅作對照）／**待評估：會議摘要**（未完整發表、有摘要但非可評讀完整研究；MECIR studies awaiting classification）。**★ 本關輸入全是『切題（已具內容）』候選（③ 三桶中的「切題」），故只分 核心／背景／會議摘要待評估三類；『全文及摘要皆無／無內容』是第③關 Tier 4 的終端桶、已在 ③ 定案，嚴禁在本關重新產生『待評估,無內容』（機器守門 `gate_guard.py check_units_no_nocontent`）。** **一定先決定，再叫使用者補全文。**　→ 停，報告分流（哪些 Study 進分析、哪些 MA 當背景）。
...
以上全部確認後**才**進入：**PDF 報告（5 段固定版型）→ 寫交接包**（每一步各自再停）。**★ PDF＝使用者定版『5 段核心』（1 檢索基本參數／2 完整檢索字串逐字／3 PRISMA 流程數字／4 納入證據清單〔只核心，欄位＝`作者／年份／文獻類型 ｜ 標題 ｜ DOI ｜ PMID ｜ 驗證`，背景不列表〕／5 進行中試驗〔NCT｜標題〕）。流程＝`EBM_Search/scripts/build_search_report_data.py`（由 cache 組 `_search_report.json`）→ `build_search_report.py`（渲染）。嚴禁手拼或自創版型。** （交叉驗證已於 ⑤a 完成、僅作標記，**不因 UNVERIFIED 刪文**。）
